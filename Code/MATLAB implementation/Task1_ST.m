...
%
% This script simulates the SUPERTREX algorithm on Task 1 as presented in
...
function y = Task1_ST(triallen, res_folder, green, red, orange, purple, grey, rseed)    % Alteration #1
    ...
    for i = 1:1
        ...
        z_mst(:,i) = W_FORCE*r(:);
        z(:,i) = z_exp(:,i) + z_mst(:,i);
        zbar = z_exp(:,i);                                                  % only exploratory path
    ...
    for i = 2:Triallen/dt * numTrain
        ...
        z_mst(:,i) = W_FORCE*r(:);
        z(:,i) = z_exp(:,i) + z_mst(:,i);
        zbar = .8*zbar + .2*z_exp(:,i);

        mse(i) = sum((z(:,i)-f(:,i)).^2);
        mse_delta = .8*mse_delta + .2*mse(i);
        
        betause = .025*(10*mse_delta)^.25;

        test = -(mse(i)-mse_delta);
        if (test >= 0)
            testuse = -1*(abs(test).^.25);
            W_RMHL = W_RMHL + 1*learningrate*(-5*testuse)*(z_exp(:,i) - zbar)*r(:)';
        else
            %testuse = -(abs(test).^.25);
            %NET2RO = NET2RO + .25*learningrate*(5*testuse)*(ro5b(:,i) - robar)*z(:)';
        end
        ...
        if (mod(i, 10) == 0)
            ...
            % NEW - weight update by error (soft threshold)
    ...
    W_RMHL = 0; % Disable exploration during testing
