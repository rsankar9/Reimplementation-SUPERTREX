% Task 2 - SUPERTREX  - 3 Segments - Variable lengths 
...
%
% This script simulates the SUPERTREX algorithm on a variant of Task 2 as presented in
...
function y = Task2_ST_Seg3_Var(triallen, res_folder, green, red, orange, purple, grey, rseed) % Alteration #1
    ...
    fs = zeros(NRO,numT); % target function
    fs(1,:) = sout2;
    fs(2,:) = sout3;
    fs = repmat(fs,1,numTrial);
    
    
    % Position of joints from output (angles) [3 segments]
    ...
    for i = 2:Triallen/dt * numTrain
        ...
        betause = .01*(10*mse_delta)^.2; % Task 2

        test = -(mse(i)-mse_delta);
        if (test >= 0)
            testuse = -(abs(test).^.25);
            W_RMHL = W_RMHL + 1*learningrate*(-5*testuse)*(z_exp(:,i) - zbar(:,i))*r(:)';
        else
            testuse = -(abs(test).^.25);
            W_RMHL = W_RMHL + 1*learningrate*(5*testuse)*(z_exp(:,i) - zbar(:,i))*r(:)';
        end
        ...
        if (mod(i, 10) == 0)
            ...
            c = 1.0/(1.0 + rPr);
            %P = P - k*(k'*c);
            P = P - k*(k'*c)*(-.5*tanh(500000*(abs(mse_delta)-(1.5e-3)))+.5);
            ...
            dw = .5*-e*k'*c*(-.5*tanh(500000*(abs(mse_delta)-(1.5e-3)))+.5);
            %dw = .5*-e*k'*c;	
            W_FORCE = W_FORCE + dw;
...
%     mkdir(res_path);
    ...
    saveas(gcf, res_path + '/CoordinateX.png');
